...
def get_unexplored_areas(explore_map, unmapped_value):
    ...
    unexplored_areas=[]
    unexplored_areas = np.argwhere(explore_map==unmapped_value)
...
def get_new_destination(current_position, unexplored_areas):
    ...
    dest = []
    minimum = sys.maxsize
    for i in unexplored_areas:
        distance1 = np.sqrt((current_position[0]-i[0])**2+(current_position[1]-i[1])**2)
...
def update_explore_map(dest, route, explore_map, planned, unmapped):
    ...
    all_unmap = get_unexplored_areas(explore_map,unmapped)
...
for itr in range(max_itr):
    ...
    for i in range(num_bots):
        fig = plt.gcf()
        ax = fig.gca()
        ax.add_patch(patches.Rectangle((bots[i]['current_position'][1] - .5, bots[i]['current_position'][0] - .5), 1, 1,
                                       edgecolor='blue', facecolor='red', fill=True))
